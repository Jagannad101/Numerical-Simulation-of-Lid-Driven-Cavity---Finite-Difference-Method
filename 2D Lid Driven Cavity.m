...
function [PrsMatrix, uvelMatrix, vvelMatrix] = cavity_template_to_students_new(~)
...
for n = ninit:nmax
    ...
    if isgs==1 % ==Symmetric Gauss Seidel==
        ...
        if isgs==0 % ==Point Jacobi==
            ...
            % Set Boundary Conditions for u_old_old
            set_boundary_conditions();
        else
...
% !************ADD CODING HERE FOR INTRO CFD STUDENTS************ */
% !************************************************************** */
dtvisc = (fourth*dx*dy*rho)/rmu;       % Viscous time step stability criteria (constant over domain)
...
for j = 2:jmax-1
    for i = 2:imax-1
        ...
        beta2(i,j) = max(uvel2(i,j),rkappa.*vel2ref);       % Beta squared parameter for time derivative preconditioning
        ...
        lambda_max(i,j) = max(lambda_x(i,j),lambda_y(i,j));  % Max absolute value eigenvalue (used in convective time step computation)
        dtconv(i,j) = min(dx,dy)./lambda_max(i,j);       % Local convective time step restriction
...
for j = 2:jmax-1
    for i = 2:imax-1
        artviscx(i,j) = -((lambda_x(i,j).*Cx.*(dx^3))./beta2(i,j)).*d4px4(i,j).*lim;
        artviscy(i,j) = -((lambda_y(i,j).*Cy.*(dy^3))./beta2(i,j)).*d4py4(i,j).*lim;
    end
end
...
for j = 2:jmax-1
    for i = 2:imax-1
        u(i,j,1) = u(i,j,1) - (beta2(i,j).*dt(i,j)).*((rho.*dudx(i,j))+(rho.*dvdy(i,j))-s(i,j,1)-artviscx(i,j)-artviscy(i,j));
    end
end
for j = 2:jmax-1
    for i = 2:imax-1
         u(i,j,2) = u(i,j,2) - (dt(i,j)*rhoinv).*((rho*u(i,j,2)).*(dudx(i,j))+(rho*u(i,j,3)).*(dudy(i,j))+dpdx(i,j)-(rmu*d2udx2(i,j))-(rmu*d2udy2(i,j))-s(i,j,2));
    end
end
for j = 2:jmax-1
    for i = 2:imax-1
...
for j = 2:jmax-1
    for i = 2:imax-1
        u(i,j,3) = uold(i,j,3) - (dt(i,j)*rhoinv).*((rho*uold(i,j,2)).*(dvdx(i,j))+(rho*uold(i,j,3)).*(dvdy(i,j))+dpdy(i,j)-(rmu*d2vdx2(i,j))-(rmu*d2vdy2(i,j))-s(i,j,3));
    end
end
...
% !************************************************************** */
res(:)=0;
R1=zeros(imax-2,jmax-2);
R2=zeros(imax-2,jmax-2);
R3=zeros(imax-2,jmax-2);
for j=2:jmax-1
    for i=2:imax-1
               
        R1(i,j)=(u(i,j,1)-uold(i,j,1))/dt(i,j);
        R2(i,j)=(u(i,j,2)-uold(i,j,2))/dt(i,j);
        R3(i,j)=(u(i,j,3)-uold(i,j,3))/dt(i,j);
               
        res(1)=res(1)+R1(i,j)^2;
        res(2)=res(2)+R2(i,j)^2;
        res(3)=res(3)+R3(i,j)^2;
        
    end
end
res(1)=sqrt(res(1)/((imax-2)*(jmax-2)));
res(2)=sqrt(res(2)/((imax-2)*(jmax-2)));
res(3)=sqrt(res(3)/((imax-2)*(jmax-2)));
if n==1
    resinit=res;
end
conv=max(res);
...
if ( (mod(n,200)==0)||(n==ninit) )
    fprintf('Iteration Iter. Time (s)   dt (s)      Continuity    x-Momentum    y-Momentum\n');
end
